...
Eden also provides a variety of skeletons for parallel processing tasks
that  makes it faster to write parallel programs, however, they are
not used in our implementation so we won't cover them here.

In our case both the process abstraction and the skeletons are too
...
\texttt{createC} creates a primitive imput channel and a handle to
access the data received via this channel. \texttt{connectToPort}
connects the outport of the thread executing the function call to a
given channel i.e. the corresponding inport. When using
\texttt{sendData} you pass a mode as well as the data to be
...
to send the values of a data stream. The Instantiate i message is sent
to start a remote process on PE i.
